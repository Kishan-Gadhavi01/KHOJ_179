...
"""
Author: Dev
Adding the functionality of blocking the roads
"""
# Parameters
TRAFFIC_THRESHOLD = 2  # Number of vehicles to consider as traffic jam
FRAME_INTERVAL = 5  # Frames between checking traffic
blocked_edges = {}  # Dictionary to keep track of blocked edges and their blocking status
# edit ends
...
# Function to identify and block edges
def manage_traffic_edges(step):
    """
    Author: Dev
    Function to block and release edges based on threshold
    """
    if step % FRAME_INTERVAL == 0:
        # Fetch all edges
        edges = traci.edge.getIDList()

        # Detect traffic jams and block adjacent edges
        for edge_id in edges:
            vehicle_count = traci.edge.getLastStepVehicleNumber(edge_id)

            if vehicle_count > TRAFFIC_THRESHOLD:
                # Identify incoming edges to this edge's junction
                junction_id = traci.edge.getFromJunction(edge_id)
                incoming_edges = traci.junction.getIncomingEdges(junction_id)

                # Block an adjacent incoming edge
                for incoming_edge in incoming_edges:
                    if incoming_edge != edge_id and incoming_edge not in blocked_edges:
                        traci.edge.setDisallowed(incoming_edge, ["all"])
                        blocked_edges[incoming_edge] = step  # Record blocking step
                        print(f"Blocked edge: {incoming_edge} at step {step}")
                        break

        # Release edges if traffic is below threshold
        edges_to_release = []
        for blocked_edge, block_step in blocked_edges.items():
            if step - block_step >= FRAME_INTERVAL:
                vehicle_count = traci.edge.getLastStepVehicleNumber(blocked_edge)
                if vehicle_count <= TRAFFIC_THRESHOLD:
                    traci.edge.setAllowed(blocked_edge, ["all"])
                    edges_to_release.append(blocked_edge)
                    print(f"Released edge: {blocked_edge} at step {step}")

        # Clean up released edges
        for edge in edges_to_release:
            del blocked_edges[edge]
...
def run_simulation(config_file, duration=1000, red_zone_data=None, safe_zone_data=None, water_logging_data=None, vehicle_data_dict=None):
    ...
    for step in range(duration):
        with traci_lock:
            ...
            # The below line handles the blocking logic
            manage_traffic_edges(step)
...
# def monitor_all_edges_and_lanes(all_edges):
#     print("Monitoring Started")
#     global traci_lock, got
#     got = []
...
if __name__ == "__main__":
    ...
    #         with traci_lock:
    #             traci.simulationStep()
    #         print(f"Monitored edges with high traffic: {got}")
